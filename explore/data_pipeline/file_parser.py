...
from llm_inference.llm_inference_base import ai_completion_with_backoff, get_length
...
def process_single_file(input_pdf_path: str, output_path: str):
    ...
    markdown_content = ai_completion_with_backoff(
        messages=[
            {"role": "user", "content": [
                uploaded_file,
...
def process_files(file_paths: List[str], output_csv_path: str, num_workers: int = 4):
    ...
    df["cleaned_markdown"] = df.parallel_apply(
        lambda row: ai_completion_with_backoff(
            messages=[
                {"role": "user", "content": [
                    row["goolge_file"],
                    PROMPT_TO_CONVERT_DOC_TO_MARKDOWN
